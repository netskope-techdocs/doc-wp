...
class Recent_Posts extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description' => esc_html__( 'Your sit  e&#8217;s most recent Posts.', 'docy' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'recent-posts', esc_html__( '(Docy) Recent Posts', 'docy' ), $widget_ops );
	...
	public function widget( $args, $instance ) {
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : esc_html__( 'Recent Posts', 'docy' );
		...
		foreach ( $r->posts as $recent_post ) :
            ...
            $title      = ( ! empty( $post_title ) ) ? $post_title : esc_html__( '(no title)', 'docy' );
            $title_length = function_exists('get_field') ? get_field( 'title_length',  'widget_'.$args['widget_id']) : '';
            $title_attr = !empty($title_length) ? "title='$title'" : '';
            ?>
            <div <?php post_class( 'media d-flex recent_post_item') ?>>
                <?php if ( has_post_thumbnail($recent_post->ID) ) :  ?>
                    <a href="<?php the_permalink($recent_post->ID); ?>" class="thumbnail">
                        <?php echo get_the_post_thumbnail( $recent_post->ID, 'docy_70x70', array( 'class' => 'media-object bs-sm' ) ); ?>
                    </a>
                <?php endif; ?>
                <div class="media-body">
                    <a href="<?php the_permalink( $recent_post->ID ); ?>" <?php echo $title_attr; ?>>
                        <h5> <?php
                            if ( !empty($title_length) ) {
                                docy_core_limit_latter($title, $title_length);
                            } else {
                                echo $title;
                            }
                            ?>
                    ...
                    <div class="meta d-flex">
                        <?php if ( function_exists('docy_reading_time') ) : ?>
                            <a href="<?php the_permalink( $recent_post->ID ); ?>">
                                <?php docy_reading_time($recent_post->ID); ?>
                            </a>
                        <?php endif; ?>
                        <?php if ( $show_date ) : ?>
                            <span class="dot"></span>
                            <a class="entry_post_date sep" href="<?php docycore_day_link() ?>">
	...
	public function form( $instance ) {
        ...
        <p><input class="checkbox" type="checkbox"<?php esc_attr(checked( $show_date )); ?> id="<?php echo esc_attr($this->get_field_id( 'show_date' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'show_date' )); ?>" />
            <label for="<?php echo esc_attr($this->get_field_id( 'show_date' )); ?>"><?php esc_html_e( 'Display Post Date?', 'docy' ); ?></label></p>
        <?php
	}
